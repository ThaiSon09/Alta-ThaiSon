...
const TableComponent: React.FC<IBEPaginationTable> = (props: IBEPaginationTable) => {
  ...
  const [state, setState] = useState<IState>({
    ...
    option: { ...InitOption, ...defaultOption },
    selection: [],
  });
  ...
  const handleChangePage = (newPagination: PaginationEntity, _filter?: any, _sorter?: any) => {
    ...
    const newCurrent = newPagination.current;
  ...
  const thisColumns = React.useMemo(() => {
    ...
    const columnTranslate = col.map(item => {
      ...
      return { ...item, title, ellipsis: true };
    });
    //xét có nên thêm stt
    if (hasStt) {
      const hasSttColumn = {
  ...
  }, [columns, hasStt, listPermissionCode, tableKey, intl, state.pagination, queryParam]);
